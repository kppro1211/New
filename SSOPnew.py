...
if(var==2):
    ...
    """IN THE KMAP, A IS VERTICAL AND B IS HORIZONTAL"""
    ...
    def right(kmap,position):
        ...
        #Identify unique groups of 2
    usedup=[]
    stringlist=[]
    for position in positionlist:
        if kmap[position[0]][position[1]]!='-1' and up(kmap, position)== 1 and right(kmap, position)==0:
            usedup.append(position)
            usedup.append(kmap[position[0]-1][position[1]])
            if position[1]==1 and 'B' not in stringlist:
                stringlist.append("B")
            elif position[0]==0 and "B'" not in stringlist:
                stringlist.append("B'")
            kmap[position[0]][position[1]]='-1'
            kmap[position[0]-1][position[1]]='-1'
        elif kmap[position[0]][position[1]]!='-1' and up(kmap, position)== 0 and right(kmap, position)==1:
            usedup.append(position)
            usedup.append(kmap[position[0]][position[1]-1])
            if position[1]==1 and "A" not in stringlist:
                stringlist.append("A")
            elif position[0]==0 and "A'" not in stringlist:
                stringlist.append("A'")
            kmap[position[0]][position[1]]='-1'
            kmap[position[0]][position[1]-1]='-1'
        print(stringlist)

    for position in positionlist:
        if position not in usedup:
            if position == (0,0):
